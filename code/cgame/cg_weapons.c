...
void CG_RegisterWeapon( int weaponNum ) {
	...
	case WP_LIGHTNING:
        ...
        switch(cg_oldLightning.integer){
            case 1:
                cgs.media.lightningShader = trap_R_RegisterShader( "lightningBoltNew");
                break;
            default:
                cgs.media.lightningShader = trap_R_RegisterShader( "lightningBoltNew3");
                break;
        }
...
void CG_DrawWeaponSelect( void ) {
	...
	if ( weaponSelect < 3 ) {
        x = 320 - count * 15;
        y = cgs.screenYmax  - 100; // - STATUSBAR_HEIGHT - 40
        dx = 30;
		dy = 0;
	} else {
        x = cgs.screenXmin + 4;
	...
	for ( i = WP_GAUNTLET ; i < MAX_WEAPONS ; i++ ) {
		...
		if ( i == cg.weaponSelect ) {
            CG_DrawPic( x-2, y-2, W_ICON_SIZE+4, W_ICON_SIZE+4, cgs.media.selectShader );
		}
